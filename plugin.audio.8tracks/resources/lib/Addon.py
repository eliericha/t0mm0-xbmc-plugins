...
def get_playlist(pl_type, new=False):
...
def add_song(song, img='', total_items=0):
    infolabels = {'title': unicode(song.get('title', get_string(30001))),
                  'artist': unicode(song.get('artist', get_string(30002))),
                  'album': unicode(song.get('album', get_string(30003))),
...
def add_album(album, img='', total_items=0):
    infolabels = {'title': unicode(album.get('title', get_string(30001))),
                  'artist': unicode(album.get('artist', get_string(30002))),
                 }
    add_directory({'mode': 'get_music_directory', 'id': album['id']}, 
                  album['title'], img, total_items)

def resolve_url(stream_url, listitem=False):
    if stream_url:
...
def build_query(queries):
    return '&'.join([k+'='+urllib.quote(unicode(v).encode('ascii', 'ignore')) 
...
if is_xbmc:
    addon = xbmcaddon.Addon(id='plugin.audio.8tracks')
